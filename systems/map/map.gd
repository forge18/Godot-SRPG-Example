...
func get_nearest_neighbor_to_unit(var unit, var unit_arr):
	var nearest_t = null
	for p in unit_arr:
		if p.curr_health <= 0: continue
		for n in p.get_tile().get_neighbors(unit.jump_height):
	...
	return nearest_t if nearest_t else unit.get_tile()


func get_weakest_unit_to_attack(var unit_arr):
	var weakest = null
	for p in unit_arr:
